...
**Испорченный указатель на код под четырёхпоточной компиляцией.** Отказ примерно
в трети прогонов пробы `JIT stress: 4 threads`. Права доступа ни при чём: адрес
отказа лежит внутри диапазона, который рантайм выделил под данные (видно в
`[protect-history]`), и адрес возврата указывает туда же. Опровергнуто
измерением: освобождение динамических методов сборщиком (удерживающая проба
падает так же) и усечение адреса до 32 бит (достроенный адрес не отображён).
Воспроизведение — три секунды, проба в `normal-hello`.
