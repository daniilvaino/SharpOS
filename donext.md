...
## Front: приостановка потоков для CoreCLR (условие вытеснения управляемого кода)

**Измерено в step156, не предположение.** Два прогона, отличавшиеся одним
флагом `Probes.PreemptHostedSession`:

- выключено — PowerShell стартует, баннер, приглашение, команды работают;
- включено — отказ доступа внутри `Enumerable.ToArray()` при `Import-Module`,
  то есть порча памяти в пути выделения.

Причина уровнем ниже: CoreCLR останавливает мир, **приостанавливая потоки и
читая их контексты**, а эти примитивы у нас заглушки —
`CRT_STUB(GetThreadContext)`, `ResumeThread` = no-op, `SuspendThread` в этом
виде отсутствует (`pal/sharpos/crt_imp_stubs.cpp`). При кооперативном
планировании их отсутствие безобидно: потоки переключались только в наших
точках уступки, и мир стоял сам собой. Вытеснение делает это предположение
ложным.

**Почему это важно, а не academic:** без вытеснения управляемого кода фоновый
поток (загрузка модулей, JIT) держит процессор, и поток ввода голодает —
наблюдается как «после команды побежали строки, ввод не реагирует».

**Сырьё уже есть:** блок контекста потока (`Thread.ContextBlock`), обход
припаркованных стеков (`RunFromParkedThread`), кадр прерывания у вытесненного
потока (`Thread.PreemptedFrame`, `RunFromInterruptFrame`). То есть нужны
`SuspendThread`/`GetThreadContext`/`SetThreadContext` поверх нашего
планировщика, а не с нуля.
