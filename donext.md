...
## Стенд: ноутбук, который гоняется с ПК без рук

Цель — цикл «собрал на ПК → ноутбук загрузил сборку → прогнал батарею →
лог на ПК» одной командой, без Windows на ноутбуке и без нажатий. Сейчас
каждый прогон на железе — человек у ноутбука; частям 2–3 плана (вытеснение,
USB) нужно много таких прогонов.

**Железо.** Ноутбук (Ryzen 3 7320U, AMI) — без RJ45, есть Type-C. Ненужный
Android-телефон, умеет USB-модем. Флешка SharpOS живёт в корневом порту.

**Сеть до SharpOS — через UEFI.**
- Первый кандидат: телефон по USB в режиме модема, сам в домашнем Wi-Fi.
  Ноутбук видит его как сетевую карту (RNDIS или CDC-NCM), получает адрес от
  телефона и через его NAT ходит к ПК. Входящих к ноутбуку нет — не нужно:
  ноутбук сам опрашивает ПК.
- Проверка: UEFI Shell с флешки, телефон с включённым модемом → `ifconfig -l`
  (есть `eth0`?) → `ifconfig -s eth0 dhcp` → `ping <ПК>`. Нет `eth0` — в
  прошивке нет драйвера RNDIS/NCM; есть в EDK2 (`UsbNetworkPkg`, BSD), но
  `.efi` придётся собрать. Проверить заодно, не выключает ли телефон модем
  при сбросе ноутбука.
- Запасной: USB-RJ45 (RTL8153/AX88179, ~500 ₽) и кабель в ПК (2.5G на B650)
  или роутер.
- USB-C напрямую ПК↔ноутбук не работает: оба хоста, USB4 нет.

**Схема без своего сетевого кода.** `startup.nsh` в Shell в цикле качает с
ПК `next.nsh` (`http http://<ПК>:8080/next.nsh fs0:\next.nsh`) и выполняет
его; на ПК маленький HTTP-сервер (PowerShell) отдаёт команды и сборки. Shell
умеет только скачивать — для отправки лога нужна крошечная UEFI-программа
(POST `\sharpos\bootlog.txt`), на C# поверх наших привязок UEFI; при нужде
вырастает в агента вместо Shell.

**Что нужно от SharpOS при любом варианте.**
- Автозапуск: файл на флешке со списком программ (AOTTESTS, BenchAot, Bench
  ×3, …), без лаунчера.
- Сброс машины в конце (порт `0xCF9` / регистр сброса ACPI) — прошивка снова
  грузит Shell, тот отправляет лог.
- Сторож: на тике «N секунд без прогресса → сброс», паника в этом режиме —
  сброс. Жёсткое зависание с выключенными прерываниями — позже аппаратный
  сторож чипсета AMD.

**Выбранный путь (2026-09-15): рутованный Oukitel WP19 как USB-гаджет.**
Ядро 4.14.186, в конфиге `USB_CONFIGFS_MASS_STORAGE`, `USB_CONFIGFS_F_HID`,
`USB_CONFIGFS_ACM`, `USB_CONFIGFS_RNDIS` = y. Телефон отдаёт ноутбуку образ
как флешку (ПК подменяет по Wi-Fi, лог читается из образа), клавиатуру
(Ctrl+Alt+Del, сценарии PowerShell) и последовательный порт ACM — живой лог
SharpOS на ПК, если научить наш USB-стек CDC-ACM. Сторож чипсета AMD + сторож
на тике + сброс в конце прогона — кнопка питания нужна только при зависании
прошивки. Цикл без человека ~1–1.5 мин.

Состояние: загрузчик разблокирован (`fastboot flashing unlock`), прошивка
`WP19_EEA`, `SP1A.210812.016` от 2022-07-21, Android 12, не A/B, есть
`vendor_boot`. Нужен `boot.img` этой сборки: либо mtkclient в BROM
(`C:\android-home\mtkvenv`, `C:\android-home\mtkclient`, UsbDk нужен;
при 1% заряда телефон вместо BROM грузится в Orange State — сначала
зарядить), либо официальная прошивка WP19 EEA из Google Drive Oukitel
(ссылка на oukitel.com/pages/download-center). Дальше Magisk: патч
`boot.img` в приложении, `fastboot flash boot`. `fastboot` — только
`C:\platform-tools-33` (новый падает на кириллице в профиле), драйвер
fastboot — Google USB Driver (`C:\android-home\usb_driver`) вручную как
«Android Bootloader Interface».

Прогресс 2026-09-15: UsbDk установлен (после установки нужна перезагрузка ПК),
mtkclient указан на официальную libusb (`libusb_package`, правка в
`mtkclient\...\usblib.py`). BROM один раз пойман: `SBC/SLA/DAA disabled`,
агент залит, встал на настройке DRAM — нужен preloader. Официальная прошивка
скачана: `C:\android-home\wp19_fw.tgz` (сборка 2025-10-13, на телефоне
2022-07-21), извлечены `preloader_k85v1_64.bin`, `boot.img`, `vbmeta.img` в
`C:\android-home\ASW2100PRO_YJ_2201_T2115\`. Режим preloader закрыт
(`Handshake failed`, телефон уходит в зарядку `PID_20FF`). Дальше:
- BROM ещё раз с `--preloader ...\preloader_k85v1_64.bin` — снимет родной
  `boot.img`. Вход в BROM: обе громкости зажаты ДО втыкания кабеля; через
  ~10 с удержания телефон сам включается — mtkclient должен успеть раньше;
- или без BROM: прошить архив целиком (fastboot) и патчить его `boot.img` —
  версии совпадут, телефон не жалко.

Со стороны SharpOS по порядку: автозапуск по файлу + сброс + «нет задания —
подождать и сбросить» + сторож на тике; сторож чипсета AMD (проверить, не
заблокирован ли прошивкой); составное USB-устройство (флешка + клавиатура);
драйвер CDC-ACM.

**Потом: xHCI Debug Capability (DbC).** Порт USB 3 контроллера ноутбука как
USB-устройство, специальный кабель USB 3.0 A–A для отладки (без VBUS; обычный
A–A порты палит). ПК видит ноутбук через WinUSB: живой лог (замена COM-порту,
которого нет, и медленной записи на флешку), клавиши с ПК, приём сборки
прямо в SharpOS. Реализуется в нашем стеке xHCI. Первый шаг — напечатать
расширенные возможности xHCI на ноутбуке и ПК: есть ли DbC.
