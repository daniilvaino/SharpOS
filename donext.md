...
## Нехватка памяти должна называть себя

`GcHeap.AllocateRaw` при исчерпании кучи возвращает `null`, и `RhpNewFast` /
`RhNewString` возвращают его дальше — прямо в скомпилированный код, который
считает результат ссылкой. Первый же, кто по ней пишет, получает #PF по нулевому
адресу, и отчёт называет **чужое** место: в step162 это был конструктор строки
тремя кадрами ниже настоящего виновника (драйвер Terminal.Gui, выделявший строку
на каждую клетку экрана).

Сборка-при-нехватке добавлена (тоже отсутствовала), так что упереться в предел
стало труднее. Но сам отчёт всё ещё неверен.

**Как надо:** `OutOfMemoryException`, созданное **заранее** — в момент нехватки
памяти на создание исключения уже нет. Ровно так делает BCL. Тогда падение
укажет на того, кто просил память, а не на того, кто первым её коснулся.
