    ...
    where TCharProvider : ICharProvider
{
    ...
    private void CheckLastClosingElements()
    {
        ...
        {
            var name = PopTagName();
            // SharpOS cut: interpolating a span would have to box it. ToString
            // is what the interpolation was going to do anyway.
            _handler.OnError("Invalid tag " + name.ToString()
                + " not closed at the end of the document.", _line, _column);
        }
    }
    ...
    private void ParseEntity(ref char c)
    {
        ...
        if (c == '#')
        {
            ...
            // SharpOS cut: System.Text.Rune is not in this std, and the two
            // things this code asked it — is the code point valid, is it in the
            // basic plane — are the definitions themselves.
            if (!IsValidCodePoint(charCodePoint))
                XmlThrowHelper.ThrowInvalidXml(XmlThrow.InvalidCharacterFound, line, column);

            AppendValidCodePoint(charCodePoint);
    ...
    private static bool IsValidCodePoint(int value)
        => (uint)value <= 0x10FFFF && (value < 0xD800 || value > 0xDFFF);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private void AppendValidCodePoint(int codePoint)
    {
        if (codePoint <= 0xFFFF)
        {
        ...
        {
            var highSurrogate = (char)(0xD7C0 + (codePoint >> 10));
