...
namespace PeNet.Header.Pe
{
    ...
    public class ImageFileHeader : AbstractStructure
    {
        ...
        public static List<string> ResolveFileCharacteristics(FileCharacteristicsType characteristics)
        {
            ...
            // SharpOS local mod: Enum.GetValues<T>() needs reflection metadata we
            // don't ship. Flag-name resolution is cosmetic and omitted; the raw
            // Characteristics value is still parsed and available.
            return st;
        }

        // SharpOS local mod: was `static readonly Dictionary = new() {...}` which
        // trips the ClassConstructorRunner cctor path (nativeaot-nostd-kernel-
        // limits.md §1). Factory property (no cached static) sidesteps it; the
        // machine-name lookup is cosmetic and off the milestone-1 parse path.
        private static Dictionary<MachineType, string> MachineNames => new Dictionary<MachineType, string>
        {
