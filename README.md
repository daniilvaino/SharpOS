...
3. **Build-time codegen через PowerShell**, когда MSVC-style линкер требует символов от C-ABI (пример - security cookie), мы генерим крохотный `.c` внутри build script, компилим, подхватываем — но в репо его нет.
...
| `string.Format` / `StringBuilder.AppendFormat` | ✅ | ✅ | ✅ | в AOT покрыты основные format-specifiers (`{0}`, `{0:D}`, `{0:X}`, `{0:F2}`, alignment `{0,10}`); culture-specific форматирование — InvariantCulture only |
| `System.Text.RegularExpressions.Regex` | 🔴 | 🔴 | ✅ | в AOT имлементация отсутствует, coreCLR покрытие полное |
| `try` / `catch` / `finally` / `throw;` / `when`-filter | ✅ | ✅ | ✅ | finally ordering под nested EH + filter-when + bare `throw;` rethrow — все работают (см. NativeAotProbe / normal-hello Sec 9) |
...
| `[ModuleInitializer]` | ✅ | ✅ | ✅ | работает на всех tier'ах; ILC dispatch'ит до user code |
...
| Generic sharing (USG — `__Canon`) | ✅ | ✅ | ✅ | reference-T shared code; value-T own instantiation; interface dispatch внутри shared-generic body работает (step 32) |
| Virtual dispatch / interface dispatch (полный резолвер) | ✅ | ✅ | ✅ | vtable lookup, sealed virtuals, override resolution, single-slot cache, DispatchMap walker, lazy TypeManager init через RTR scan (step 32) |
| Write barrier (`RhpAssignRef`, `RhpStelemRef`) | ✅ (no-op) | ✅ (no-op) | ✅ | non-generational mark-sweep в AOT → barrier семантически no-op (просто `*dst = src`); контракт ILC соблюдён. Hosted CoreCLR — generational, реальный card-table |
...
| `System.Collections.Concurrent.*`, `System.Collections.Immutable.*`, `SortedDictionary`, `SortedSet`, `BitArray`, `KeyedCollection`, `Array.BinarySearch`| 🔴 | 🔴 | ✅ | в AOT еще не реализовано, при этом известных блокеров - нет |
...
| Self-modifying shellcode без cpuid-serializer | Kernel | ⏳ | патчеры пишут template из `.rdata` (step 118 — через `BootAsm.Generator`) и сразу зовут без cpuid serializing; QEMU forgiving, реальное железо может выполнить stale prefetch |
...
- **[Iced](https://github.com/icedland/iced)** (icedland, MIT) — x86/x64 encoder. Используется в двух режимах: (1) `BootAsm.Generator`-ом для compile-time codegen kernel-шеллкодов на этапе сборки (step 117/118), (2) baked-in в kernel image для runtime fluent-API shellcode emission после того как boot закончился (step 115).
