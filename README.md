...
| `System.Threading.Thread.Start()` | ✅ `Scheduler.Spawn` | ⏳ E8 (deferred) | ⏳ E9 (WIP) | |
| `Interlocked.CompareExchange` (real atomic) | ✅ через `X64Asm.CmpXchg64` | 🟡 single-thread stub | ⏳ E9 | |
| Cooperative `Yield()` / `Sleep(ms)` | ✅ | ⏳ E8 | ⏳ E9 | |
| `Event` / `Semaphore` / `Mutex` | ✅ | ⏳ E8 | ⏳ E9 | |
| Multi-thread Process | ✅ logical | ⏳ E8 | ⏳ E9 | no MMU isolation в любой phase |
...
**Note про 🚫.** Серьёзная философская разница между 🔴 и 🚫: 🔴 означает "когда-нибудь сделаем если понадобится", 🚫 — "не сделаем потому что выход за пределы design'а SharpOS как unikernel'а / NativeAOT'а". Если 🚫 фича нужна — это **другой проект**, не доработка SharpOS'а.
