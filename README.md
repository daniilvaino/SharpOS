...
| **Generic `as T` / `(T)x` с `where T : class`** | 🚫 | 🚫 | 🚫 | generic RhTypeCast helper не вытянут |
...
| `Task.Run` | ⏳ | ⏳ | ✅ | |
| `Task.Delay` | ⏳ | ⏳ | 🟡 | висит: TimerQueue не подключен + `Wait(timeout)` не honour'ит deadline |
...
| **Parallel execution at same VA** | 🚫 | 🚫 | 🚫 | следствие выше |
