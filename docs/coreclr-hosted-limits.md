...
## 4b. Вытеснение управляемого кода — ✅ с оговоркой (step158)
...
**Оговорка.** Примерно в трети прогонов четырёхпоточной компиляции — отказ на
испорченном указателе на код. Права страниц ни при чём: адрес отказа лежит
внутри диапазона, который рантайм сам выделил под данные. Версии «сборщик забрал
динамический метод» и «усечение адреса до 32 бит» опровергнуты измерением.
