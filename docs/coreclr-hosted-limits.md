...
- 🟡 Исключения: ~59 мкс на бросок, ×1.5 к Linux в том же QEMU (step170; было
  120 мкс). Сняты: замеры ядра по HPET на пути размотки, побайтовые
...
- 🟡 На железе (step172–173, ноутбук против Windows на нём же, тёплое к
  тёплому, сессия без переписи — `Probes.HostedRunAtBoot = false`):
  выделение, сборка gen0 и полная, строки, коллекции, JIT — вровень или
  быстрее; исключения ×1.2–1.9; `tasks` ×2–6 (одно ядро); PowerShell
  упирается в чтение с флешки. После переписи куча 106 МиБ и 144 тыс.
  объектов в очереди финализации — каждая сборка gen0 стоит ~10 мс.
  Таблицы — `done/step172.md` §8–9, `done/step173.md`.
- ✅ Тик 1000 Гц (step173; был 100): задачи 15 → 3 мкс, эхо клавиш в
  PowerShell < 20 мс (было 20–46).
...
## 4b. Вытеснение управляемого кода — ✅ (step158)

Гейт `Probes.PreemptHostedSession` **включён**. Под вытеснением стабильно
проходят: счёт на четырёх потоках с самопроверкой (27–51 тыс. раундов),
компиляция одним потоком (~1600 методов) и четырьмя (~4000–4800, две пробы),
перепись `OK=152 DEG=2 FAIL=18`, запуск приложения и `exitCode=42`. Пробы
`Preemption` и `PreemptedAlloc` — PASS (вторая падала до шага).

Что для этого потребовалось — шесть дефектов одного рода, код, не защищавший
ничего, потому что при добровольном переключении защищать было не от чего:

1. правка таблиц страниц и выдача кадров без сериализации;
2. `Scheduler.Exit` считал пустую очередь готовых тупиком — после перехода
   ожиданий на настоящую блокировку это норма;
3. проверка-и-постановка в очередь в `Event.Wait` двумя шагами → потерянное
   пробуждение;
4. **взаимного исключения в рантайме не было вообще** — `EnterCriticalSection`
   и прочие были пустыми телами; первым попался распределитель кучи кода
   `EECodeGenManager::AllocCodeWorker`. Реализовано в `HostLocks.cs`, PAL —
   только переходники;
5. освобождение в куче ядра без защиты при защищённом выделении;
6. `Halt` и `FailFast` были пустым циклом: поток оставался готовым, до
   безопасной точки не доходил, и сборщик ждал его вечно (`act=2753/1/3358`).

**Седьмой дефект — подмена адреса возврата.** Рантайм останавливает поток в
скомпилированном коде, подменяя на стеке адрес возврата на свою заглушку
(`Thread::HijackThread`). Восстановление клало адрес не в ту ячейку, и заглушка
уходила по сохранённому **результату метода**: проба с меткой `0x5EED0000|номер`
падала с `RIP = 0x5EED0AD6`. Выключено точечно в `GetReturnAddressHijackInfo`
под `TARGET_SHARPOS`; `FEATURE_HIJACK` целиком не снимался. Цена: приостановка
идёт через безопасные точки и активацию, то есть медленнее. Подтверждение —
десять прогонов подряд без отказа (до правки отказ был примерно в трети).

Диагностика: `[prof]` печатает замеры по потокам, `yieldsInCrit`/`critDepth` для
критических участков и `act=` для активаций; отказ выборки инструкции печатает
происхождение страницы. Символизация — `sharpos.map` (`/MAP` в `OS.csproj`).

## 4c. Предкомпиляция и Ctrl+C (step159)

**ReadyToRun — ✅, но только своего формата.** Рантайм грузит образы того же
номера формата, что и он сам (`readytoruninfo.cpp`: минимум = максимум = 16 для
.NET 10). Дистрибутив PowerShell 7.5 нёс формат 10, и все его сборки
отвергались: `System.Management.Automation` (19 МБ) компилировался заново при
каждом запуске — это и был долгий старт и пауза на первой команде. Решение —
PowerShell 7.6.5 (`net10.0`, формат 16), выкладывается из `payloads\pwsh\` через
`run_build.ps1`. `System.Private.CoreLib` остаётся наш: он и рантайм — одно целое.

Диагностика: строки `[r2r] <сборка>: accepted|<причина>` по одной на модуль,
категория `TraceGate.R2r`. Видны при выключенном `HostedAppQuietConsole`.

**Ctrl+C — ✅ (step159).** Отменяет выполняющуюся команду. Потребовалось три
вещи: передавать ядру указатель зарегистрированного обработчика (форк его
выбрасывал), забирать клавиши потоком `InputPump` независимо от читателя (обе
клавиатуры опрашиваются по запросу, а во время команды не спрашивает никто), и
доставлять событие с потока, известного рантайму — `SharpOSHost_AttachCurrentThread`,
знакомство отложено до первой доставки. Реализован `FlushConsoleInputBuffer`:
обработчик зовёт его первым, и его отсутствие валило оболочку.
...
### ✅ `#XM` на VirtualBox — ЗАКРЫТ

`HwFaultBridge` строил CONTEXT из аппаратной ловушки и ставил `CONTEXT_FULL`, то
есть «здесь есть и FP-состояние», при **нулевой** области `FltSave` (в коде так и
было: «Xmm registers — leave 0»). CoreCLR верит флагам и делает restore → `MXCSR = 0`
→ **сняты маски всех исключений SSE**. Дальше любая операция с плавающей точкой даёт
`#XM`, включая безобидную (`mulsd` при пересчёте байтов: со снятыми масками ловушку
вызывает даже неточный результат).

**Fix:** `CONTEXT_CONTROL | CONTEXT_INTEGER` вместо `CONTEXT_FULL`.

**Правило:** нули для FP-состояния — это не «пусто», а конкретная и вредная
конфигурация. Не объявлять того, чего не заполняем.

**Почему только VBox:** QEMU/TCG не возбуждает `#XM` за незамаскированные исключения
SSE — дефект был общим и латентным, наказывала только аппаратная виртуализация.
Ср. §13/§14 в `nativeaot-nostd-kernel-limits.md` (видеопамять, xHCI) — тот же класс.

**Побочно снят мнимый второй дефект:** тройная ошибка в раскрутке стека при выводе
из обработчика была тем же корнем — `#XM` внутри dispatch поверх уже обрабатываемой
ловушки. После фикса прогон с `HostedAppQuietConsole = false` чист на обеих машинах.

**Проверено на железе:** census `OK=147 DEG=2 FAIL=18`, exitCode=42; интерактивный
PowerShell 7.5.5 с USB-клавиатурой. Тир по-прежнему IL-only (без R2R), планирование
кооперативное.
