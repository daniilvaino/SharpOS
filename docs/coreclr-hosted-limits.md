...
Корень — не в разметке критических участков ядра (она сделана: кучи, часы,
PS/2, диск, USB, консоль), а в том, что CoreCLR останавливает мир
приостановкой потоков, а `GetThreadContext`/`ResumeThread`/`SuspendThread` в
PAL — заглушки. Кооперативное планирование делало их отсутствие безобидным.
