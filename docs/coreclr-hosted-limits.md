...
## 4b. Вытеснение управляемого кода — ❌ (step156)

Ядро умеет вытеснять свои потоки по таймеру (step155). К hosted-ярусу это
**не применимо**: с включённым вытеснением сессия умирает отказом доступа
внутри управляемого выделения (`Enumerable.ToArray` в `Import-Module`).

Корень назывался неверно (step156): не заглушки `GetThreadContext`/
`ResumeThread` — форк определяет `TARGET_UNIX`, `DISABLE_THREADSUSPEND` включён
и `Thread::SuspendThread` вырезан, эти заглушки никто не зовёт. Механизм —
**активация потока**, и она реализована в step157 (`act=2/1/15` подтверждает
доставку).

Мешает другое: предположения о порядке, верные только без вытеснения. По
очереди выпали нереентерабельная консоль, щель между проверкой и постановкой в
очередь у семафора, и выставление прав на страницы JIT пачкой (отказ выборки
инструкции на NX-странице). Это фронт, а не одна поломка.

Следствие для отзывчивости: фоновый поток держит процессор, поток ввода
голодает. Снимается только реализацией приостановки — см. `donext.md`.

Диагностика: `Probes.SampleProfiler` печатает `[prof]` со счётом замеров по
потокам; один поток, забирающий почти все замеры, — подпись этой проблемы.
