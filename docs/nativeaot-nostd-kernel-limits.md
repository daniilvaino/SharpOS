...
## 10. Kernel GC sweep — нельзя звать из произвольного места

### ❌ `KernelGC.Collect()` из callsite без `GcRoots.CaptureStackTop()` прямо
перед — порождает random bit-flip corruption памяти.

**Корень.** Mark phase делает conservative stack scan в окне
`[currentRSP, s_stackTop)`. `s_stackTop` — global static, который
устанавливается одноразово через `GcRoots.CaptureStackTop()`. Если caller
не вызвал capture **в той же frame'е** прямо перед `Collect`,
`s_stackTop` указывает в zone стека от **давно popped'нутой frame'ы**
(например GcHeapSmokeTest из ранней Phase4 — он capture'ит и сразу
Collect'ит, дисциплина соблюдена). Sweep сам по себе ок; проблема в
Mark walker'е: видит мусор от popped frame'ов как «pointers in
GcHeap», деref'ит как managed-object, идёт по GcDesc, ставит mark-bit
(bit 0 RawMethodTable) на wild адреса. В коде/JIT-памяти это single-
bit-flip → corrupted instruction → wild jump → `#GP` с non-canonical
RIP. Наблюдалось при попытке звать Collect из `BootSequence.PhaseReport`:
RIP=`0xD0000000001CA1CB` на Phase5 ELF launcher после Phase4 Collect.

**Правильная дисциплина** (как в `GcHeapSmokeTest.Run` /
`GcStressTest.Run`):
```csharp
public static void Run()
{
    // ...сделать allocations, разместить ссылки в registered roots
    // или в локалах текущего frame'а...
    GcRoots.CaptureStackTop();   // снимок границы scan window
    KernelGC.Collect();          // scan = [Collect.RSP, here] — узкое окно
}
```

**Почему дисциплина слабая.** Это не enforce'ится компилятором: легко
забыть. Поэтому Collect де-факто не должен зваться из «обычного» кода
— только из явных GC-aware тестов / probes. Production-путь сейчас НЕ
зовёт `GC.Collect()` нигде — память монотонно растёт, что приемлемо
для kernel-tier текущего размера.

**Долгосрочный фикс (план):** один из:
1. **Precise GcInfo per stack frame** (как у настоящего JIT) — устраняет
   conservative scan вообще.
2. **Strict MT validation** в walker'е — проверять что MT принадлежит
   зарегистрированной таблице известных MethodTable'ов (RTR scan
   построил TypeManager — можно использовать) перед dereference'ом.
3. **Explicit-roots-only mode** — отключить conservative stack scan,
   требовать чтобы все managed refs регистрировались. Меняет
   программирующую модель.

До закрытия одного из этих пунктов: `KernelGC.Collect` нельзя звать
без CaptureStackTop, и фактически он не зовётся в production.
