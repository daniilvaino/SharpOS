...
### ⚠️ Чтения статика вокруг выделения сворачиваются в одно

**Симптом:** тест меряет, во что обошлось выделение, — снимает счётчик до и
после `new byte[N]` — и разность всегда ноль. Выглядит как «работа не
выполнялась»: свободный список не тронут, сегмент не вырос, ничего не
произошло. На деле всё произошло.

```csharp
ulong before = GcHeap.FreelistProbeCount;   // чтение статика
byte[] big = new byte[4096];                // newarr -> helper
ulong delta = GcHeap.FreelistProbeCount - before;   // всегда 0
```

**Корень:** ILC моделирует `newarr`/`newobj` как выделение, которое **не пишет
пользовательские статики**. Это верно для любого аллокатора, кроме нашего:
`RhpNewArray` — наш собственный код, и счётчики ведёт он же. Компилятор об этом
не знает и сворачивает два чтения в одно.

Любой непрозрачный вызов между ними ломает сворачивание — отсюда самый
коварный признак: **добавление печати рядом «чинит» измерение**. Три подряд
неверных диагноза в step178 пришли отсюда; каждый раз прибор, поставленный
ближе к месту, менял результат.

**Fix:** геттеры диагностических счётчиков — `[MethodImpl(NoInlining)]`
(`GcHeap`, `GcSweep`). Невстраиваемый вызов свернуть нельзя. `volatile` не
годится: C# запрещает его для `ulong`.

**Правило для тестов:** если разность счётчика вокруг действия равна нулю, это
сначала подозрение на сворачивание и только потом на то, что действие не
выполнилось. Проверяется добавлением непрозрачного вызова — если разность
изменилась, виноват прибор.

### ⚠️ Счётчик HPET бывает 32-битным и обходится за 300 с (step177)

**Симптом:** на ноутбуке (`64bit=no`, 14.318 МГц) машина встаёт примерно через
полторы минуты работы лаунчера. Тики идут, дампы печатаются, все потоки в
`Waiting` и ни одно значение не меняется часами. В QEMU (`64bit=yes`) не
воспроизводится никогда.

**Корень:** `ReadCounter` читал 64 бита и утверждал в комментарии, что верхняя
половина нулевая. Верно ровно до первого обхода. Дальше всякое
`deadline = now + delta` указывает туда, куда счётчик уже не попадёт: `Sleep`,
`Monitor.Wait` с таймаутом, `WaitOnAddress`, `WaitForExit`, `ComputeDeadline`
CoreCLR (все его конечные ожидания), спины AHCI и xHCI, калибровка APIC.

**Fix:** эпоха в `Hpet.s_latch`, шаг — **знаковая** 32-битная разница
(`Hpet.Extend`); двигают её все читатели попутно и тик APIC раз в 64 такта.
Запас — половина диапазона (150 с). `TryForceRestart` обнуляет защёлку вместе
со счётчиком. **Не SMP-корректно** — нужен seqlock перед вторым ядром.

**Приложениям** мало сырого адреса: в хвосте `AppServiceTable` появились
`HpetCounterBits` и `HpetLatchAddress`, SDK-`Stopwatch` повторяет формулу и
защёлку только читает.

**Тесты:** `hpet.extend` / `hpet.wrap` (`HpetWrapProbe`, фаза 3 — драйвер поверх
буфера, два обхода за микросекунды) и `CheckClockWrap` в `AotTests`.
