...
**Батарея прошла границу app-tier рантайма: 14/14 (step139 закрыл interface dispatch).** Freestanding PE-апп несёт **минимальный** рантайм (GC + прямые вызовы); interface dispatch **разблокирован через handoff** (см. ниже), EH пока нет (нет типа `System.Exception`, ThrowHelpers = halt → **Tier-B halt-on-throw**).
...
| `throw` / `catch` | ❌ (Tier-B) | Нет `System.Exception` + managed-EH требует per-image `.pdata` registration (§EH ниже) |
...
**EH — следующий фронт (тот же handoff-паттерн + одна kernel-работа):** managed-EH (`RhpThrowEx`→`DispatchEx`) шарится handoff'ом (адреса `RhpThrowEx`/funclet-энтри в service table; `ExInfoHead.s_head` — kernel-глобал, ок при single-thread ExInfo). Матч catch-типа (`IsAssignableFromClass`) уже pure/app-safe. Блокер — **managed function-table single-image**: `CoffRuntimeFunctionTable` зашит на kernel-образ, весь путь (`CoffMethodLookup`/`CoffEhDecoder`/`StackFrameIterator`) читает один `ImageBase`; кадры аппы (RVA от 0x400000) не находятся. Нужно: multi-image реестр `(imageBase, records, count)` (шаблон — `SehUnwind.s_stat`/`RegisterStaticFunctionTable` из другого движка) + `PeLoader` регистрирует `.pdata` аппы (подтверждено: AotTests.exe несёт `.pdata`, 61 запись) + `System.Exception` в app Compile list.
