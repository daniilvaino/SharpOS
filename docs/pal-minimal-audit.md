...
> **Статус на 2026-08-22 (сверено по коду форка).**
> **Закрыто:** пункт 1 — всё семейство `__stdio_common_vsprintf*` идёт через
> `vsnprintf_impl` (`crt_imp_stubs.cpp:1156`), диагностического затемнения
> больше нет: сообщения об исключениях в цензе настоящие, а не `?`.
> **Открыто:** пункт 2 — `_wassert` по-прежнему печатает и продолжает вместо
> `noreturn`, и это осознанно (иначе каскад `abort → TerminateProcess` гасит
> машину до следующей эмпирической стены). Пункт 3 — `__stdio_common_vsscanf`
> так и возвращает 0 без пометки; ловушки на неподдержанном пути нет.
> Средние и долгосрочные пункты (маркировка подмножеств, фазовые аннотации,
> grep-CI) не делались.
>
> Из «списка не в PAL» устарела одна строка: cross-image walk **реализован**
> (step140), см. `docs/eh-model.md`.
