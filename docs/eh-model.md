...
- ~~**Cross-image walk.**~~ **Сделано (step140):** `CoffRuntimeFunctionTable`
  держит реестр `(ImageBase, .pdata)` на образ, и раскрутка проходит кадры
  ядра и приложения в одном обходе. Ловушка, которую это открыло: базу
  образа обязан отдавать тот, кто нашёл метод (`info.ImageBase`), а не
  глобальная — иначе адрес таблицы раскрутки считается от чужой базы и
  указывает в стек (step165).
