...
- ядра/загрузочного образа (`OS_0.1`);
...
Новые общие no-runtime компоненты (строки, corelib-поверхность, runtime-контракты) добавлять сюда, а не размазывать по `OS_0.1/src/*` и `apps/sdk/*`.
