...
## Довесок: SUPER-2 фаза 1 — GC layout types в std/

Создан `std/no-runtime/shared/GC/` с тремя файлами, адаптированными из Gosse (MIT):
- `MethodTable.cs` (`GcMethodTable`) — парсер NativeAOT EEType. Обнаружили что layout отличается от Gosse's CoreCLR версии: NativeAOT Flags это `uint16 @ offset 2`, не `uint32 @ offset 0`. Оставили только verified поля: ComponentSize, Flags, BaseSize, RelatedType. `HasComponentSize` реализован через `ComponentSize != 0` (универсально).
- `GcDescSeries.cs` — `GcDescSeries`, `ValSerieItem` для описания reference-полей (данные для Mark phase, используются в фазе 3).
- `GcObject.cs` (`GcObject`) — layout объекта с mark-bit в lower bit MethodTable pointer. Методы: `Mark/Unmark/IsMarked`, `ComputeSize`.

MinimalRuntime (оба) — добавлены `AttributeTargets.Field`, `FieldOffsetAttribute`.

Smoke-test в Kernel.cs validated что парсинг работает для string/int/object/int[]/object[]: `ComponentSize`, `BaseSize`, `HasComponentSize` корректные, `RelatedType` читается (но semantics определяем позже).
